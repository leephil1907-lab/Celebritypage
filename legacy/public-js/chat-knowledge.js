...
(function(){
  ...
  const KB = [
    ...
    { q: ["admin","cms","Kimura Management"], a: "KIMURA MANAGEMENT (back-office): manages Homepage NOW campaign, Work/music/tours/journal/archive, Members/fan cards, Fan content, Shop/support, Notifications, Media/campaigns/analytics — all without code. Roles: Super Manager, Management, Editor, Tour Mgr, Membership Mgr, Support Agent, Shop Mgr, Moderator, Analyst, Developer — strict perms + audit logs." }
  ];
      ...
      if(ans){
        ...
        setTimeout(()=>{
          ...
          if(last && last.textContent.includes('You')){
            ...
            div.innerHTML = `Got it — I’ll forward this to <b>Management</b>. Tap <b>Open ticket → Management</b> to create a ticket #ID; Management replies here + via email.<div style="margin-top:8px"><button class="btn gold small" style="padding:6px 10px;font-size:10px" onclick="window.openTicketFromChat()">Open ticket → Management</button></div>`;
            el.appendChild(div);
            el.scrollTop = el.scrollHeight;
          }
        }, 1100);
